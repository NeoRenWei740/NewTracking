...
with tab_tracker:
        ...
        if not ST_USER or not ST_PASS:
            ...
            try:
                ...
                if not tle_data or not ref_tle_data:
                    ...
                    # We collect both 'lon' (absolute) and 'rel_lon' (relative)
                    plot_data = {sat: {'epoch': [], 'inc': [], 'raan': [], 'ecc': [], 'arg_pe': [],
                                        'mean_anom': [], 'mean_mo': [], 'semi_major': [], 'lon': [], 'rel_lon': [], 'dist': []}
                                 for sat in sat_list}
                    ...
                    for i in range(0, len(lines), 2):
                        ...
                        # Absolute Longitude
                        cand_lon = sat_obj.at(t).subpoint().longitude.degrees
                        plot_data[nid]['lon'].append(cand_lon)

                        ref_sat_best = best_tle(ref_tles, t_dt)
                        
                        if ref_sat_best:
                            # Relative Longitude
                            ref_lon = ref_sat_best.at(t).subpoint().longitude.degrees
                            rel_lon = (cand_lon - ref_lon + 180) % 360 - 180
                            plot_data[nid]['rel_lon'].append(rel_lon)
                            
                            plot_data[nid]['dist'].append(
                                euclidean_km(tuple(sat_obj.at(t).position.km),
                                             tuple(ref_sat_best.at(t).position.km)))
                        else:
                            plot_data[nid]['rel_lon'].append(None)
                    ...
                    if all_sats_data:
                        ...
                        # Explicitly exclude 'rel_lon' from the raw data table and CSV export
                        cols = ['NORAD_ID', 'epoch', 'dist', 'lon', 'inc', 'raan', 'ecc', 'arg_pe',
                                'mean_anom', 'mean_mo', 'semi_major']
                        st.session_state['raw_data_df'] = combined_df[cols]
    ...
    if st.session_state.get('data_ready'):
        ...
        with charts_tab:
            ...
            # --- 3-Axis Graph (Time, Distance, Relative Longitude) ---
            st.markdown(f"### 🔀 Distance & Relative Longitude Correlation (vs. {ref_id})")
            ...
            for idx, sat in enumerate(sat_list):
                ...
                # Plot Relative Longitude (Right Y-Axis)
                fig_combo.add_trace(go.Scatter(
                    x=plot_data[sat]['epoch'], y=plot_data[sat]['rel_lon'],
                    name=f"Sat {sat} (Rel. Lon)", mode='lines+markers', 
            ...
            fig = make_subplots(
                ...
                subplot_titles=(
                    ...
                    "Mean Anomaly (°)", "Mean Motion (rev/day)", "Relative Longitude (°)", 
            ...
            for idx, sat in enumerate(sat_list):
                ...
                # Plotting 'rel_lon' in the visual chart instead of 'lon'
                params = [('inc', 1), ('raan', 2), ('ecc', 3), ('arg_pe', 4),
                          ('mean_anom', 5), ('mean_mo', 6), ('rel_lon', 7), 
                          ('dist', 8), ('semi_major', 9)]
        ...
        with data_tab:
            ...
            if csv_data:
                ...
                if 'raw_data_df' in st.session_state:
                    st.dataframe(st.session_state['raw_data_df'], use_container_width=True, height=500)
            else:
